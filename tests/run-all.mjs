// 전체 검증: 빌드 → 규칙 단위 테스트 → 서버 로직 → 재접속/관전/정원 → 서버 재시작 복원 → 2인 UI 플레이 → AI 연습 판 → 3인 구역 격리 → 에디터
...
run('규칙 단위 테스트 (카드 대전 세션 — 타이머·기권·난이도)', 'npx tsx tests/unit-cb-session.test.ts');
...
run('카드 대전 재시작 복원', 'node tests/smoke-cb-restart.mjs');
...
run('카드 대전 AI 연습 판 (브라우저 UI)', 'node tests/play-cb-solo.mjs');
